...
def merge_one_session(session: UploadSession):
    ...
    if not parts:
        ...
        print(f"[merge_worker] No parts found for file fingerprint={file.fingerprint}, "
    ...
    if len(parts) != session.total_chunks:
        ...
        print(f"[merge_worker] Incomplete parts for file fingerprint={file.fingerprint}, "
    ...
    print(f"[merge_worker] Start merging file fingerprint={file.fingerprint}, "
    ...
    try:
        ...
        with open(tmp_path, "wb") as out_f:
            for part in parts:
                ...
                if not os.path.exists(chunk_path):
                    ...
                    print(f"[merge_worker] Chunk file missing: {chunk_path}, abort merge.")
        ...
        print(f"[merge_worker] Merge success for fingerprint={file.fingerprint}, "
              f"final_path={final_path}")

    except Exception as e:
        # 任何异常都打印出来，回退状态避免死循环
        traceback.print_exc()
        print(f"[merge_worker] Merge failed for fingerprint={file.fingerprint}: {e}")
